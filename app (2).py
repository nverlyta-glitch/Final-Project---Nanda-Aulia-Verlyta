...
def load_data():
    try:
        ...
        df['InvoiceDate'] = pd.to_datetime(df['InvoiceDate']).dt.date  # Konversi ke date untuk konsistensi
...
rfm = filtered_df.groupby('CustomerID').agg({
    'InvoiceDate': lambda x: (today - x.max()).days,  # FIXED: Hapus .date() karena x.max() sudah date
...
# Konversi sementara ke datetime untuk .dt.to_period (karena date object tidak punya .dt)
